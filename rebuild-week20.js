...
async function run() {
  ...
  console.log("====================================");
  console.log("REBUILDING WEEK 20");
  ...
  // IMPORTANT:
  // This ONLY creates the rebuilt Week 20 file.
  // It DOES NOT overwrite the current live weekly-leaders.json.
  ...
  console.log("");
  console.log("====================================");
  console.log("WEEK 20 REBUILD COMPLETE");
  console.log(`Wrote: ${outPath}`);
  console.log(`Scoring period: ${dates[0]} through ${dates[dates.length - 1]}`);
  ...
  if (overallLeaders.length > 0) {
    console.log(
      `#1 overall: ${overallLeaders[0].name} — ${overallLeaders[0].totalPoints}`
    );
  }
...
run().catch((error) => {
  console.error("WEEK 20 REBUILD FAILED:");
  console.error(error);
  process.exit(1);
});
